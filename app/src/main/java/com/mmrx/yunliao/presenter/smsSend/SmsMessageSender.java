...
//                DEFAULT_DELIVERY_REPORT_MODE);
        boolean requestDeliveryReport = true;

        for (int i = 0; i < mNumberOfDests; i++) {
            try {
                L.v(TAG, "queueMessage mDests[i]: " + mDests[i] + " mThreadId: " + mThreadId);

                MessageUtils.addMessageToUri(mContext.getContentResolver(),
                ...
                L.e(TAG, "queueMessage SQLiteException ");

                e.printStackTrace();
            }
        }
        // Notify the SmsReceiverService to send the message out
        Intent intent = new Intent(SmsService.ACTION_SEND_MESSAGE,
                null,
                mContext,
                SmsReceiver.class);
    ...
    private String getOutgoingServiceCenter(long threadId) {
        ...
        try {
            cursor = mContext.getContentResolver().query(
                    Uri.parse(Constant.SMS_INBOX_URI), SERVICE_CENTER_PROJECTION,
